                        ...
                        For Each vs In Values(r.Value.Item1)
                            If vs.Key.IsUInteger Then
                                ...
                                Catch ex As OverflowException
                                    AdvResult.Fault.Add(New INIAnalizeInfo(r.Value.Item2, "特遣小队成员ID严重超出规定范围，可能导致AI行为混乱或崩溃", r.Value.Item1, r.Key))
                                    Continue For
                                End Try
                                If CUInt(vs.Key) > 6 Then
                                    AdvResult.Warning.Add(New INIAnalizeInfo(r.Value.Item2, "特遣小队成员ID大于6，此成员会被忽略", r.Value.Item1, r.Key))
                                Else
                                    Dim team = vs.Value.Item1.Split(","c)
                                    If team.Count <> 2 Then
                                        AdvResult.Fault.Add(New INIAnalizeInfo(r.Value.Item2, "特遣小队成员格式无效，应为数量,单位，可能导致运行时AccessViolation异常", r.Value.Item1, r.Key))
                                    ElseIf team(0).IsUInteger Then
                                        If Not RulesUnitsCache.Contains(team(1)) Then
                                            AdvResult.Fault.Add(New INIAnalizeInfo(r.Value.Item2, "特遣小队成员单位未注册，可能导致运行时AccessViolation异常", r.Value.Item1, r.Key))
                                        End If
                                    Else
                                        AdvResult.Fault.Add(New INIAnalizeInfo(r.Value.Item2, "特遣小队成员数量格式无效，应为正整数，可能导致运行时AccessViolation异常", r.Value.Item1, r.Key))
                                    End If
                                End If
                            End If
                        Next
                    Else
                        AdvResult.Warning.Add(New INIAnalizeInfo(r.Value.Item2, "特遣注册了，但是没有定义，此注册会被忽略", r.Value.Item1, "TaskForces"))
                    End If
                Next
