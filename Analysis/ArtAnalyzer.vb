                    ...
                    Select Case r.Key
                        Case "Sequence"
                            If Not Values.ContainsKey(r.Value.Item1) Then
                                AdvResult.Fault.Add(New INIAnalyzeInfo(r.Value.Item2, "使用了不存在的动作序列，可导致运行时AccessViolation异常。", r.Value.Item1, Record.Key))
                            End If
                        Case "Trailer", "Next"
                        ...
                        Case "TiberiumSpawnType"
                            ...
                            If r.Value.Item1.Length <= 2 OrElse Not r.Value.Item1.Substring(r.Value.Item1.Length - 2).IsUInteger Then
                                AdvResult.Fault.Add(New INIAnalyzeInfo(r.Value.Item2, "产生的覆盖物名的结尾不是两位数字，可导致运行时AccessViolation异常。", r.Value.Item1, Record.Key))
                            End If
                        Case "Warhead"
                            If Not rules.Values.Keys.Contains(r.Value.Item1) Then
                                AdvResult.Fault.Add(New INIAnalyzeInfo(r.Value.Item2, "使用了不存在的弹头，可导致运行时AccessViolation异常。", r.Value.Item1, Record.Key))
                            End If
                        Case "SpawnsParticle"
