...
Public MustInherit Class Ra2IniAnalyzer
    ...
    Sub New(INIText As String)
        MyBase.New(INIText)
    End Sub
    Protected Sub TypeCheck(Arr As IEnumerable(Of String), ids As IEnumerable(Of Integer), chk As Func(Of String, Boolean), errtxt As String, lst As IList(Of INIAnalyzeInfo), Linenum As Integer, MKName As String)
        For Each n In ids
            If Not chk.Invoke(Arr(n)) Then
                lst.Add(New INIAnalyzeInfo(Linenum, String.Format(errtxt, n), Arr(n), MKName))
            End If
        Next
    End Sub
    ...
    Protected Sub ValueRegistryCheck(MainKeyName As String, FailText As String, MainKeyOfCheck As String, Record As KeyValuePair(Of String, Tuple(Of String, Integer)), lst As IList(Of INIAnalyzeInfo), Optional SecondaryMainKey As String = Nothing)
    ...
    ''' <param name="Record">正在扫描的记录</param>
    ''' <param name="lst">目标列表</param>
    Protected Sub MainKeyRegistryCheck(MainKeyName As String, FailText As String, MainKeyOfCheckingKV As String, Record As KeyValuePair(Of String, Tuple(Of String, Integer)), lst As IList(Of INIAnalyzeInfo), Optional SecodaryMainKeyName As String = Nothing)
        ...
        If 0 = Aggregate c In From v In Values(MainKeyName) Where v.Value.Item1 = MainKeyOfCheckingKV Into Count Then
            If String.IsNullOrEmpty(SecodaryMainKeyName) Then
                lst.Add(New INIAnalyzeInfo(Value.Item2, FailText, Value.Item1, MainKeyOfCheckingKV))
            Else
    ...
    Protected Sub EachValueRegistryCheck(r As KeyValuePair(Of String, Tuple(Of String, Integer)), ls As List(Of INIAnalyzeInfo), MK As String, Tx As String, FindMKey As String, Optional SecondaryMainKey As String = Nothing)
